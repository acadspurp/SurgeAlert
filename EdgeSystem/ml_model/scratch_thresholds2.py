...
def run_balanced():
    ...
    for t2 in np.linspace(0.001, 0.2, 50):
        for t1 in np.linspace(0.001, 0.2, 50):
            y_pred = np.zeros(len(probs), dtype=int)
            for i, p in enumerate(probs):
                if p[2] > t2: y_pred[i] = 2
                elif p[1] > t1: y_pred[i] = 1
                else: y_pred[i] = 0
    ...
    print(f"t2={best_t2:.4f}, t1={best_t1:.4f} -> Macro Recall: {best_rec:.2%}, Macro F1: {best_f1:.2%}")
