...
export default function DashboardView(props) {
    ...
    const formatManilaTime = (value) => new Date(value).toLocaleTimeString('en-US', {
        hour: '2-digit',
        minute: '2-digit',
        timeZone: 'Asia/Manila'
    });

    const formatManilaDate = (value) => new Date(value).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
        timeZone: 'Asia/Manila'
                                    ...
                                    <div key={i} className="flex items-start gap-3 bg-[#0f172a] border border-slate-700 rounded-xl p-3">
                                        ...
                                        <div className="flex-1">
                                            <div className="text-xs font-bold text-slate-200">
                                                {log.timestamp ? `${formatManilaDate(log.timestamp)} ${formatManilaTime(log.timestamp)}` : '—'}
                                            </div>
                        ...
                        <div className="flex-1 bg-gradient-to-b from-[#0f172a] to-[#111827] rounded-xl p-6 flex flex-col justify-center text-center border border-slate-700">
                            {nextTide ? (
                                <>
                                    ...
                                    <p className="text-3xl font-black text-cyan-300 my-2">{formatTideTimeDelayed(nextTide.dt * 1000)}</p>
                                    <p className="text-xs font-semibold text-slate-300 mb-2">{formatTideDateDelayed(nextTide.dt * 1000)}</p>
