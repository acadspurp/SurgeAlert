...
const data = fs.readFileSync('src/pages/Admin/index.jsx', 'utf8');

// I want to wipe out the broken gap between where HANDLERS should begin
// and the User Management handlers.
const headMarker = '} else if (level === \\'GREEN\\') newDash.statusColor = \\'text-green-600\\';'; 
// Wait, no. I'll just use the `// "Last updated" ticker` 
const start = data.indexOf('    // "Last updated" ticker');
const end = data.indexOf('    // User Management Modal Handlers');

if (start === -1 || end === -1) {
    console.log('Cant find start or end', start, end);
    process.exit(1);
}

const safeBlock = `    // "Last updated" ticker
    useEffect(() => {
        if (!lastMqttAt) {
...
const newStr = data.substring(0, start) + safeBlock + data.substring(end);
fs.writeFileSync('src/pages/Admin/index.jsx', newStr);
